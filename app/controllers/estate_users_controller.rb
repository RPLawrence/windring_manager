class EstateUsersController < ApplicationController
  before_action :set_estate_user, only: [:show, :edit, :update, :destroy]

  # GET /estate_users
  # GET /estate_users.json
  def index
    @estate_users = EstateUser.all
  end

  # GET /estate_users/1
  # GET /estate_users/1.json
  def show
  end
  ...
  # POST /estate_users
  # POST /estate_users.json
  def create
    @estate_user = EstateUser.new(estate_user_params)

    respond_to do |format|
      if @estate_user.save
        format.html { redirect_to @estate_user, notice: 'Estate user was successfully created.' }
  ...
  # PATCH/PUT /estate_users/1
  # PATCH/PUT /estate_users/1.json
  def update
    respond_to do |format|
      if @estate_user.update(estate_user_params)
        format.html { redirect_to @estate_user, notice: 'Estate user was successfully updated.' }
  ...
  # DELETE /estate_users/1
  # DELETE /estate_users/1.json
  def destroy
    @estate_user.destroy
    respond_to do |format|
  ...
  private
    ...
    def estate_user_params
      params.require(:estate_user).permit(:EstateID, :uuid)
    end
end
