...
class TokensControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create token" do
    assert_difference('Token.count') do
      post tokens_url, params: { token: { UUID: @token.UUID, token: @token.token, validity: @token.validity } }
    end
  ...
  test "should update token" do
    patch token_url(@token), params: { token: { UUID: @token.UUID, token: @token.token, validity: @token.validity } }
