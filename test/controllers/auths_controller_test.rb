...
class AuthsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create auth" do
    assert_difference('Auth.count') do
      post auths_url, params: { auth: { accountType: @auth.accountType, passwordHash: @auth.passwordHash, passwordSalt: @auth.passwordSalt, webLoginKey: @auth.webLoginKey } }
    end
  ...
  test "should update auth" do
    patch auth_url(@auth), params: { auth: { accountType: @auth.accountType, passwordHash: @auth.passwordHash, passwordSalt: @auth.passwordSalt, webLoginKey: @auth.webLoginKey } }
